function go_home()
	if first == 0 then
		until_found(800,color.游戏开始)
		found_tap(color.游戏开始,point.跳过点)
	end
	first = 0
	until_found(800,color.开始界面)
	while true do
		mSleep(800)
		tapT(point.开始游戏)
		mSleep(800)
...
function thumbs_up()
	...
	t1,x, y = until_found(800,color.点赞确认)
	found_tap(color.点赞确认,{x,y})
	until_found(800,color.点赞界面)
	none_tap(color.主界面,point.主界面)
end
function donate()
	...
	if x > -1 then
		tap(x, y)
		until_found(800,color.捐赠数量界面)
		tapT(point.MAX)
		mSleep(800)
		tapT(point.捐赠确认)
		t1, x, y = until_found(800, color.捐赠确认)
		tap(x, y)
		until_found(800,color.行会界面)
	end
	x, y = mfColor(color.可捐a)
	if x > -1 then
		tap(x, y)
...
function daily_task()
	...
	if x > -1 then
		...
		t1, x, y = until_found(800,color.邮件确认)
		found_tap(color.邮件确认,{x, y})
		t1, x, y = until_found(800,color.扫荡完成)
		found_tap(color.扫荡完成,{x, y})
		t1, x, y = until_found(800,color.关闭邮箱)
